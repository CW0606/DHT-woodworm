            ...
            if state:
                ...
                try:
                    ...
                    if encoding:
                        ...
                        sql = "update hash_info set hash_info.info='%s' where hash_info.hash='%s'" % (
                        name.decode(encoding), r[0].decode('utf-8'))
                        # print sql
                        cur.execute(sql)
                    else:
                        print name
                        ...
                        sql = "update hash_info set hash_info.info='%s' where hash_info.hash='%s'" % (
                        name.decode('utf-8'), r[0].decode('utf-8'))
                        # print sql
                        cur.execute(sql)
                    conn.commit()
                except:
